...
(define (simplify-ae e)
  ...
  (match e
         ...
         [`(,fae ,args ...)
          (define ret (wrap-aes `(,fae ,@args) (lambda (xs) xs)))                     
            ret
          ]))
...
(define (proc->llvm procs)
  ...
  (define (e->llvm e)
    (match e
           [`(let ([,x '#t]) ,e0)
            (string-append
             (comment-line
              ...
              "quoted #t")
             (comment-line  "call void @check_mem(i64 8)" "memcheck")
             (e->llvm e0))]
           [`(let ([,x '#f]) ,e0)
            (string-append
             (comment-line
           ...
           [`(let ([,x ',(? string? dat)]) ,e0)
            ...
            (string-append
             (comment-line
              ...
              "quoted string")
             (comment-line  "call void @check_mem(i64 " (number->string (+ 1 (string-length dat))) ")" "memcheck")
             (e->llvm e0))]
           [`(let ([,x ',(? symbol? dat)]) ,e0)
            ...
            (string-append
             (comment-line
              ...
              "quoted string")
             (comment-line  "call void @check_mem(i64 " (number->string (+ 1 (string-length (symbol->string dat)))) ")" "memcheck")
             (e->llvm e0))]
           [`(let ([,x (make-closure ,lamx ,xs ...)]) ,e0)
            ;(display (string-append "make: " (symbol->string x) "\n"))
            ...
            (apply string-append
                   `(,(comment-line
                       ...
                       "malloc")
                     ,(comment-line  "call void @check_mem(i64 " (number->string (* (+ (length xs) 1) 8)) ")" "memcheck")
                     ,@(map (lambda (iptr n)
                              (comment-line
           ...
           [`(clo-app ,fx ,xs ...)
            (define wrong-number "")
            ;(display (string-append "app: " (symbol->string fx) "\n"))
            ;(define expected (hash-ref argcounts fx))
            ;(when (> expected (length xs)) (display "Too many arguments"))
