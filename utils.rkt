...
(define (simplify-ir ir-e)
  (define (T e)
    (match e
           ...
           [`(prim / ,e0 ,e1)
            `(prim / ,(T e0) ,(T e1))]
      ;;;;; New desugarings for final
           [`(prim make-hash)
            (simplify-ir `(prim make-hash (prim list)))]
           [`(prim hash-ref ,h ,k ,f)
            `(prim hash-ref-flag ,h ,k ,f)]
           [`(prim make-vector ,s)
            `(prim make-vector ,s '0)]
...
(define (eval-llvm llvm-str test-name)
  ...
  (system (string-append clang++-path " combined.ll " " -o " test-name))
  (match-define `(,out-port ,in-port ,id ,err-port ,callback) (process (string-append "./" test-name)))
...
(define (test-proc->llvm proc->llvm exp prog test-name)
  (define llvm (proc->llvm prog))
  (define val0 (eval-llvm llvm test-name))
  (define val (eval-top-level exp))
  (if (or (equal? val val0) (eq? val 'failure))
      #t
      (begin
...
(define (racket-compile-eval e)
  (define (rewrite-match e)
    ...
    (match e [`(match ,e0 ,clauses ... (,pat0 ,es ...))
             ...
             [else e]))
  ;wrap call in an engine so memory tests terminate
  (define t (engine (lambda (_) (with-handlers ([exn:fail? (lambda (x) 'failure)])
                 (parameterize ([current-namespace (make-base-namespace)])
                               ...
                               (eval (compile
                                      `(call/cc (lambda (exit+)
                                                  ...
                                                  ,(rewrite-match e))))))))))
  (if (engine-run 120000 t) (engine-result t) 'failure)'failure)
